...
class PortfolioMetrics():
    ...
    #        total_realized = 0
            ...
            for ticker in selected_transactions['ticker'].unique():
                ...
                # Total invested
                invested = buys['transact_amount_base_currency'].sum()
                # Cost average per share
                cost_average = total_cost_base_currency / quantity if quantity else 0
                # Calculate realized gains/losses
   #             realized = self._compute_ticker_realized_gain_loss(buys, sales)
    ...
    #            position_pl[ticker] = self._compute_returns(position_values[ticker], position_realized_gains_losses[ticker], position_total_invested[ticker])
